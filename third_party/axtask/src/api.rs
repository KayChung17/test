...
cfg_if::cfg_if! {
    if #[cfg(feature = "sched-rt")] {
        /// Number of RT priority levels (0–99, where 99 is highest).
        pub const N_RT_PRIO: usize = 100;
        const MAX_TIME_SLICE: usize = 5;
        pub(crate) type AxTask = axsched::RtTask<TaskInner, N_RT_PRIO>;
        pub(crate) type Scheduler = axsched::RtScheduler<TaskInner, N_RT_PRIO, MAX_TIME_SLICE>;
...
pub use axsched::{SCHED_FIFO, SCHED_OTHER, SCHED_RR};

/// Get the scheduling policy of the current task.
#[cfg(feature = "sched-rt")]
pub fn get_sched_policy() -> i32 {
    current().policy()
}

/// Set the scheduling policy of the current task.
#[cfg(feature = "sched-rt")]
pub fn set_sched_policy(policy: i32) {
    current().set_policy(policy);
}

/// Get the RT priority of the current task.
#[cfg(feature = "sched-rt")]
pub fn get_rt_priority() -> i32 {
    current().priority() as i32
}

/// Set the RT priority of the current task (also updates the scheduler).
#[cfg(feature = "sched-rt")]
pub fn set_rt_priority(prio: i32) {
    current().set_priority(prio as u8);
    let _ = set_priority(prio as isize);
}
