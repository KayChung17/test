...
impl ListenTable {
    ...
    pub fn can_listen(&self, port: u16, reuse_addr: bool) -> bool {
        reuse_addr || self.tcp[port as usize].lock().is_none()
    }

    pub fn listen(&self, listen_endpoint: IpListenEndpoint, reuse_addr: bool) -> AxResult {
        ...
        if entry.is_none() {
            *entry = Some(Box::new(ListenTableEntryInner::new(listen_endpoint)));
            ...
            // SO_REUSEADDR: overwrite the existing listener
            *entry = Some(Box::new(ListenTableEntryInner::new(listen_endpoint)));
            Ok(())
        } else {
