...
/*
 * Rely on stdio.h's stdout declaration.  We only need fflush(stdout) to
 * compile, and our weak fflush() implementation below ignores the argument.
 * Defining a fallback stdout object here conflicts with newer musl headers
 * that declare `stdout` with toolchain-specific qualifiers.
 */
